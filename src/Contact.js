...
function Contact() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!capRegex.test(first)) {
      setSuccess("First name should start with a capital letter.");
      return;
    }
    if (!capRegex.test(last)) {
      setSuccess("Last name should start with a capital letter.");
      return;
    }
    ...
    if (!phoneRegex.test(phone)) {
      setSuccess("Phone number must be exactly 10 digits.");
      return;
    }
  ...
  body: JSON.stringify({ first, last, phone, email, message }),
      });
  ...
  setForm({ first: "", last: "", phone: "", email: "", message: "" });
        ...
        setSuccess(result.error || "Failed to send message.");
      }
    } catch (err) {
          ...
          <div className="contact-form" style={{ fontFamily: "Times New Roman, Times, serif" }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center', alignItems: 'center' }}>
                <select value={siteLang} onChange={e => { setSiteLang(e.target.value); localStorage.setItem('agri_lang', e.target.value); }} style={{ padding: '0.4rem', borderRadius: '0.5rem' }} title="Site language">
                ...
                <select value={siteLang} onChange={e => { setSiteLang(e.target.value); localStorage.setItem('agri_lang', e.target.value); }} style={{ padding: '0.4rem', borderRadius: '0.5rem', display: 'none' }} title="Recognition language">
