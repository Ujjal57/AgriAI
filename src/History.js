...
export default function History() {
  ...
  const openInvoice = (order) => {
    ...
    // Compute delivery and labour for the invoice
    ...
    (order.items || []).forEach((it, idx) => {
      html += `
        <tr>
          ...
          <td style="text-align:center">${it.crop_name}</td>
          <td style="text-align:center">${translateVar(it.variety)}</td>
          <td style="text-align:center">${it.order_quantity}</td>
          <td style="text-align:center">₹${it.price_per_kg}</td>
          <td style="text-align:center">₹${Number(it.total).toFixed(2)}</td>
        </tr>
      `;
    });
    ...
    html += `
          ...
          <h3 style="text-align:right;margin-top:10px;color:#000;">
            <span style="color:#236902;">${t('grandTotalLabel', siteLang) || 'Grand Total'}: ₹${Number(totals.grand_total).toFixed(2)}</span>
          </h3>
    ...
    <div style={{ background: '#53b635', minHeight: '100vh', color: '#fff' }}>
      ...
      <main style={{ padding: '6rem 1rem 2rem' }}>
        <div style={{ maxWidth: 980, margin: '0 auto', background: '#fff', padding: '1.5rem', boxShadow: '0 8px 24px rgba(0,0,0,0.06)' }}>
          <h1 style={{ color: '#236902', textAlign: 'center' }}>{t('historyTitle', siteLang) || 'Purchase History'}</h1>
          ...
          {filtered.length === 0 ? (
            <div style={{ textAlign: 'center', marginTop: 24 }}>
              ...
              <div style={{ marginTop: 8 }}>{t('historyNoPurchases', siteLang) || 'No matching purchases yet.'}</div>
            </div>
          ) : (
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                          <thead>
                            <tr>
                              ...
                              <th style={{ border: '1px solid #ddd', padding: 8, background: '#f4f4f4' }}>{t('tableVariety', siteLang) || 'Variety'}</th>
                          ...
                          <tbody>
                            {(o.items || []).map((it, idx) => (
                              <tr key={idx}>
                                ...
                                <td style={{ border: '1px solid #eee', padding: 8, textAlign: 'center' }}>{translateVar(it.variety)}</td>
