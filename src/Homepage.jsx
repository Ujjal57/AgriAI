...
import {
  ArrowRight, BarChart2, Bell, ChevronDown, FileText,
  ...
  return (
    <group>
      ...
      <Float speed={1.5} rotationIntensity={0.3} floatIntensity={0.5}>
        <Html position={[2.8, 1.2, 0]} center>
          <div style={{ fontSize: "28px", filter: "drop-shadow(0 0 8px #00ff88)" }}>🌾</div>
        </Html>
      </Float>
      <Float speed={2} rotationIntensity={0.2} floatIntensity={0.4}>
        <Html position={[-2.6, 0.8, 1]} center>
          <div style={{ fontSize: "24px", filter: "drop-shadow(0 0 8px #d4a04a)" }}>🌽</div>
        </Html>
      </Float>
      <Float speed={1.8} rotationIntensity={0.4} floatIntensity={0.6}>
        <Html position={[1.5, -2.2, 1.2]} center>
          <div style={{ fontSize: "22px", filter: "drop-shadow(0 0 6px #00ff88)" }}>🍅</div>
        </Html>
      </Float>
      <Float speed={2.2} rotationIntensity={0.3} floatIntensity={0.5}>
        <Html position={[-1.8, 2.4, -0.5]} center>
          <div style={{ fontSize: "26px", filter: "drop-shadow(0 0 8px #44ffaa)" }}>🌿</div>
        </Html>
      </Float>
...
const STATS = [
  { value: 146, suffix: "M+", label: "Farm Holdings in India", icon: Users },
  { value: 8.5, suffix: "%", label: "CAGR Contract Farming Market", icon: TrendingUp },
  { value: 30, suffix: "%", label: "Higher Income vs Traditional", icon: BarChart2 },
  { value: 2.5, suffix: "L Cr", prefix: "₹", label: "Addressable Market Size", icon: Globe },
  { value: 500, suffix: "+", label: "Agribusiness Companies", icon: FileText },
  { value: 25, suffix: "", label: "States with Legal Framework", icon: Shield },
];
...
const FEATURES = [
  { icon: Globe, title: "Direct Market Access", desc: "Eliminate middlemen. Connect farmers directly with verified buyers across 25+ states for better price discovery.", color: "oklch(0.65 0.22 145)" },
  { icon: Zap, title: "AI-Powered Crop Matching", desc: "Our AI analyzes crop profiles, buyer requirements, and market trends to match farmers with the best contracts.", color: "oklch(0.75 0.14 75)" },
  { icon: FileText, title: "Digital Contract Management", desc: "Legally binding digital contracts with e-signature, automated reminders, and dispute resolution workflows.", color: "oklch(0.65 0.22 145)" },
  { icon: TrendingUp, title: "Price Transparency", desc: "Real-time market price feeds, negotiation history, and fair price benchmarks powered by AGMARKNET data.", color: "oklch(0.75 0.14 75)" },
  { icon: Bell, title: "Government Scheme Alerts", desc: "Automatic notifications for PM-Kisan, Fasal Bima Yojana, and 50+ state-level agricultural subsidies.", color: "oklch(0.65 0.22 145)" },
  { icon: MessageSquare, title: "Multi-language Support", desc: "Platform available in English, Hindi and Kannada.", color: "oklch(0.75 0.14 75)"},
];

const TESTIMONIALS = [
  { name: "Ramesh Patel", role: "Wheat Farmer", location: "Punjab", avatar: "RP", quote: "Before this platform, I had no idea what price my wheat would get until harvest day. Now I have a contract signed 4 months in advance with a guaranteed ₹2,100/quintal.", stars: 5 },
  { name: "Sunita Devi", role: "Vegetable Grower", location: "Maharashtra", avatar: "SD", quote: "The AI matched me with a food processing company in Pune. My income increased by ₹45,000 this season compared to selling at the local mandi.", stars: 5 },
  { name: "Vikram Aggarwal", role: "Procurement Manager, AgroFoods Ltd", location: "Delhi NCR", avatar: "VA", quote: "We've onboarded 340 verified farmers in just 6 months. The contract module saves our legal team approximately 2 weeks of paperwork per procurement cycle.", stars: 5 },
  { name: "Lakshmi Narayanan", role: "Spice Farmer", location: "Karnataka", avatar: "LN", quote: "Finally a platform where I don't need a middleman. The government scheme alerts helped me discover a subsidy on drip irrigation I didn't know existed.", stars: 5 },
];
...
export default function HomePage() {
  ...
  // Translate chart data based on current language
  const translatedIncomeData = [
    { name: t('homePageTraditional', siteLang), income: 18500 },
    { name: t('homePageContractFarming', siteLang), income: 24100 },
    { name: t('homePageOrganicContract', siteLang), income: 31200 },
  ];
  ...
  return (
    <div className="min-h-screen bg-dark overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8 text-sm font-bold">
            {["Home", "How It Works", "Features", "Data Insights"].map((item) => (
              <a key={item} href={`#${getSectionId(item)}`} className="text-white font-bold hover:text-neon transition-colors">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/login" className="px-4 py-2 text-sm font-bold text-white border border-neon rounded-lg hover:bg-primary/10 transition-colors">{t('homePageLogin', siteLang)}</Link>
            <Link to="/signup" className="px-4 py-2 text-sm font-bold bg-primary text-white rounded-lg hover:opacity-90 transition-opacity glow-neon">{t('homePageGetStarted', siteLang)}</Link>
          </div>
        </div>
      </nav>
      ...
      <section id="home" className="relative min-h-screen flex items-center pt-16 grid-bg">
        ...
        <div className="max-w-7xl mx-auto px-6 w-full">
          <div className="grid lg:grid-cols-2 gap-16 items-center py-20">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, ease: "easeOut" }}>
              ...
              <p className="text-white font-bold text-lg leading-relaxed mb-8 max-w-xl">
                {t('homePageDesc', siteLang)} <span className="text-gold font-semibold">{t('homePageFarmers', siteLang)}</span>.
              </p>
              <div className="flex flex-wrap gap-4">
                <Link to="/signup" className="inline-flex items-center gap-2 px-6 py-3 bg-primary text-primary-foreground font-semibold rounded-xl hover:opacity-90 transition-all glow-neon">
                  {t('homePageStartFarmer', siteLang)} <ArrowRight className="w-4 h-4" />
                </Link>
                <Link to="/signup" className="inline-flex items-center gap-2 px-6 py-3 border border-accent/60 text-gold font-semibold rounded-xl hover:bg-accent/10 transition-all">
                  {t('homePageStartBuyer', siteLang)} <ArrowRight className="w-4 h-4" />
                </Link>
              </div>
            </motion.div>
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1 text-white font-bold text-xs">
          <span>{t('homePageScrollExplore', siteLang)}</span>
      ...
      <section id="data-insights" className="py-24 bg-dark border-y border-border">
        <div className="max-w-7xl mx-auto px-6">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-16">
            <h2 className="font-display text-4xl font-bold text-foreground mb-4">{t('homePageOpportunityTitle', siteLang)}'s Agricultural <span className="text-neon">{t('homePageOpportunity', siteLang)}</span></h2>
            <p className="text-white font-bold max-w-xl mx-auto">{t('homePageRealData', siteLang)}</p>
          </motion.div>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
            {STATS.map((stat, i) => {
              const labelKeys = ['homePageStat1Label', 'homePageStat2Label', 'homePageStat3Label', 'homePageStat4Label', 'homePageStat5Label', 'homePageStat6Label'];
              return (
              <motion.div key={stat.label} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }} className="glass-card rounded-2xl p-6 text-center">
                ...
                <div className="text-white font-bold text-sm">{t(labelKeys[i], siteLang)}</div>
              </motion.div>
            );})}
          </div>
        </div>
      ...
      <section id="how-agriai-works" className="py-24">
        <div className="max-w-7xl mx-auto px-6">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-16">
            <h2 className="font-display text-4xl font-bold text-foreground mb-4">{t('homePageHowItWorks', siteLang)} <span className="text-neon">AgriAI</span> {t('homePageWorks', siteLang)}</h2>
            <p className="text-white font-bold max-w-xl mx-auto">{t('homePageSoilToContract', siteLang)}</p>
          </motion.div>
      ...
      <section id="platform-features" className="py-24 bg-dark border-y border-border">
        <div className="max-w-7xl mx-auto px-6">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-16">
            <h2 className="font-display text-4xl font-bold text-foreground mb-4">{t('homePageFeaturesTitle', siteLang)} <span className="text-gold">{t('homePageFeatures', siteLang)}</span></h2>
            <p className="text-white font-bold max-w-xl mx-auto">{t('homePageFeaturesDesc', siteLang)}</p>
          </motion.div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {FEATURES.map((feat, i) => {
              const titleKey = `homePageFeature${i + 1}`;
              const descKey = `homePageFeature${i + 1}Desc`;
              return (
              <motion.div key={titleKey} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }} className="glass-card rounded-2xl p-6 group hover:border-primary/40 transition-colors flex flex-col items-center text-center">
                <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-5 border" style={{ background: `${feat.color}18`, borderColor: `${feat.color}30` }}>
                  <feat.icon className="w-5 h-5" style={{ color: feat.color }} />
                </div>
                <div className="flex flex-col items-center justify-center mb-2">
                  <h3 className="font-display text-lg font-bold text-foreground">{t(titleKey, siteLang)}</h3>
                  {feat.badge && <span className="text-xs px-2 py-0.5 rounded-full border border-accent/40 text-gold mt-2">{t('homePageComingSoon', siteLang)}</span>}
                </div>
                <p className="text-white font-bold text-sm leading-relaxed">{t(descKey, siteLang)}</p>
              </motion.div>
              );
            })}
          </div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.15 }} className="glass-card rounded-2xl p-6">
              ...
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={translatedIncomeData}>
                  <CartesianGrid strokeDasharray="3 3" stroke="oklch(0.25 0.04 160)" />
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="max-w-3xl mx-auto">
            <motion.div key={currentTestimonial} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.5 }} className="glass-card rounded-2xl p-8 text-center">
              ...
              <p className="text-foreground text-lg leading-relaxed mb-8 italic">"{TESTIMONIALS[currentTestimonial].quote}"</p>
              <div className="flex items-center justify-center gap-4">
                ...
                <div className="text-left">
                  ...
                  <div className="text-white font-bold text-sm">{TESTIMONIALS[currentTestimonial].role} · {TESTIMONIALS[currentTestimonial].location}</div>
                </div>
              </div>
            </motion.div>
      ...
      <section className="py-32 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-6 text-center relative">
          <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
            <h2 className="font-display text-5xl font-bold text-foreground mb-6">Ready to Transform <span className="text-neon">Your Farm's Future?</span></h2>
            <p className="text-white font-bold text-xl mb-10 max-w-2xl mx-auto">Join thousands of farmers and buyers already using AgriAI to build fair, transparent, and profitable agricultural partnerships.</p>
      ...
      <footer className="bg-dark border-t border-border py-12">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col items-center text-center mb-10">
            <div className="mb-12">
              <div className="flex items-center justify-center gap-2 font-display font-bold text-xl mb-4">
                <div className="w-7 h-7 rounded-lg bg-primary/20 border border-primary/40 flex items-center justify-center">
                  <Leaf className="w-3.5 h-3.5 text-neon" />
                </div>
                <span className="text-neon">AgriAI</span>
              </div>
              <p className="text-white text-sm leading-relaxed max-w-md mx-auto">Empowering India's 146 million farmers with transparent, technology-driven agriculture contracts.</p>
            </div>
              ...
              {[
                ...
                <div key={col.title}>
                  ...
                  <ul className="space-y-2">
                    {col.links.map((link) => (
                      <li key={link}><a href="/" className="text-white text-sm hover:text-neon transition-colors">{link}</a></li>
                    ))}
                  </ul>
                </div>
              ))}
          ...
          <div className="border-t border-border pt-6 flex justify-center items-center text-white text-sm">
            <span>© {new Date().getFullYear()} AgriAI. All rights reserved.</span>
          </div>
        </div>
      </footer>
    </div>
